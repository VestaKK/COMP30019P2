...
v2f vert(MeshData v)
{
    ...
    o.viewDir = normalize(_WorldSpaceCameraPos - o.wPos);
    TRANSFER_VERTEX_TO_FRAGMENT(o);                     // Unity gives us information on light and light positions
    return o;
}
...
fixed4 frag(v2f i) : SV_Target
{
    ...
    // Necessary normals for each fragment
    ...
    // float towardsLightChange = fwidth(NdotL); (Might need this later)
    ...
    float4 DiffuseLight = fAtt * lightIntensity * _LightColor0 * _OutColor;
    ...
    float RimColor = fAtt * rimIntensity * _RimColor;

    return  (AmbientLight + DiffuseLight + SpecularLight + RimColor) * shadow;
}